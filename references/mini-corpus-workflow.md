...
1. Choose keyed OpenAlex, anonymous OpenAlex, arXiv, or a useful combination autonomously (before this unattended phase starts, the user has already been told that they may leave the task and may not be available to answer another provider-choice question). Key configuration is the only provider decision that requires user interaction, so complete it before the unattended operation or continue without it.
