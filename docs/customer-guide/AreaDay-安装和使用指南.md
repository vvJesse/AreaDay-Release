...
- **写入本机目录**：领域注册表、复习状态、OpenAlex 配置和词向量模型都存放在本 Skill 目录下的 `data/` 文件夹里，运行环境（`.runtime`、`.venv`）也在本 Skill 目录内；建立词表与生成简报时还会写入你确认的工作区目录。也就是说这个 Skill 只需要一个可写目录：它自己所在的位置。这些位置只由 Skill 自己所在目录决定，没有任何环境变量能够改变它们；如果 Skill 被放在只读位置（例如只读挂载的沙箱），请先把它复制到可写目录再安装。
- **本机端口（打开工作台时）**：工作台会在本机 `127.0.0.1` 启动一个只对本机开放的小型服务（优先 8765 端口），再用浏览器打开页面。若你的助手默认禁止访问本机端口，打开工作台这一步会失败，需要为它放开权限；如果沙箱只允许某个特定端口，可以用 `--port 端口号` 让工作台固定使用它。
...
工作区里保存着你确认过的每一篇论文取舍、术语、词表来源和每条决定的原因；这些文件的清单和各自用途写在 Skill 的 `references/workspace-files.md` 里，助手可以随时打开对照。
